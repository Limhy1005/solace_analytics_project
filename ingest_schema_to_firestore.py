...
{{
  ...
  "purpose_summary": "A concise but specific description of the business purpose. Explicitly mention the specific data grain if applicable."
  "TimeGrain": "Describe the time level (e.g., 'Daily', 'Monthly', 'Yearly'). If the table has no time dimension, output 'None'.",
  "EntityGrain": "Describe the primary entity level or primary key grain (e.g., 'ProductKey', 'StoreKey', 'One row per transaction')."
}}
...
- Pay attention to Date dimensions or date columns to determine 'TimeGrain'.
- For Dimension tables, 'EntityGrain' is usually their primary key (e.g., ProductKey).
"""
...
def classify_all_tables(client, schema_data):
    ...
    for i, (table_name, table_data) in enumerate(sorted(tables.items()), 1):
        ...
        classified_tables[table_name] = {
            ...
            "TimeGrain": classification.get('TimeGrain', 'None'), 
            "EntityGrain": classification.get('EntityGrain', 'Unknown'),
